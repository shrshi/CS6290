...
{
    const char *block_states[11] = {"X","I","S","E","O","M","IS","IM","SM","OM","OML"};
...
void MOESI_protocol::process_cache_request (Mreq *request)
{
	switch (state) {
        ...
        case MOESI_CACHE_OML: do_cache_OML(request); break;
    default:
        fatal_error ("Invalid Cache State for MOESI Protocol in cache\n");
    }
}
...
void MOESI_protocol::process_snoop_request (Mreq *request)
{
	switch (state) {
        ...
        case MOESI_CACHE_OML: do_snoop_OML(request); break;
    default:
    	fatal_error ("Invalid Cache State for MOESI Protocol in snoop\n");
    }
}
...
inline void MOESI_protocol::do_cache_OML (Mreq *request)
{
    switch(request->msg){
        case LOAD:
        case STORE:
...
inline void MOESI_protocol::do_snoop_IS (Mreq *request)
{
    switch(request->msg){
        ...
        case DATA:
            send_DATA_to_proc(request->addr);
            if(get_shared_line()) state = MOESI_CACHE_S;
...
{
    switch(request->msg){
        case GETS: set_shared_line(); break;
        case GETM: state = MOESI_CACHE_I; break;
        case DATA: 
            fatal_error ("Should not see data for this line!  I have the line!");
            break;
        default:
...
inline void MOESI_protocol::do_snoop_SM (Mreq *request)
{
    switch(request->msg){
        ...
        case DATA:
            send_DATA_to_proc(request->addr);
            state = MOESI_CACHE_M; //O or M or OM would have sent this
            break;
        default:
            request->print_msg (my_table->moduleID, "ERROR");
            fatal_error ("Client: S state shouldn't see this message\n");
...
inline void MOESI_protocol::do_snoop_OML (Mreq *request)
{
    switch(request->msg){
        case GETS:
        case GETM: break;
