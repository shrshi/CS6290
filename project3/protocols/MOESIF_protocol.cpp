...
{
    const char *block_states[14] = {"X","I","S","E","O","M","F","IS","IM","SM","OM","OML","FML"};
...
void MOESIF_protocol::process_cache_request (Mreq *request)
{
	switch (state) {
        ...
        case MOESIF_CACHE_FML: do_cache_FML(request); break;
        case MOESIF_CACHE_OM: do_cache_OM(request); break;
        case MOESIF_CACHE_OML: do_cache_OML(request); break;
    default:
        fatal_error ("Invalid Cache State for MOESIF Protocol in cache\n");
    }
}
...
void MOESIF_protocol::process_snoop_request (Mreq *request)
{
	switch (state) {
        ...
        case MOESIF_CACHE_FML: do_snoop_FML(request); break;
        case MOESIF_CACHE_OML: do_snoop_OML(request); break;
    default:
    	fatal_error ("Invalid Cache State for MOESIF Protocol in snoop\n");
    }
}
...
inline void MOESIF_protocol::do_cache_FML (Mreq *request)
{
    switch(request->msg){
        case LOAD:
        case STORE:
...
inline void MOESIF_protocol::do_cache_OML (Mreq *request)
{
    switch(request->msg){
        case LOAD:
        case STORE:
...
inline void MOESIF_protocol::do_snoop_IS (Mreq *request)
{
    switch(request->msg){
        ...
        case DATA:
            send_DATA_to_proc(request->addr);
            if(get_shared_line()) state = MOESIF_CACHE_S;
...
{
    switch(request->msg){
        case GETS: //both F and O cannot exist simultaneously!
            set_shared_line();
            send_DATA_on_bus(request->addr, request->src_mid);
            break;
        case GETM: //both F and O cannot exist simultaneously!
            send_DATA_on_bus(request->addr, request->src_mid);
            state = MOESIF_CACHE_I;
            break;
        case DATA:
...
inline void MOESIF_protocol::do_snoop_S (Mreq *request)
{
    switch(request->msg){
        case GETS: break;
        case GETM: state = MOESIF_CACHE_I; break;
        case DATA: 
            fatal_error ("Should not see data for this line!  I have the line!");
            break;
        default:
...
inline void MOESIF_protocol::do_snoop_SM (Mreq *request)
{
    switch(request->msg){
        ...
        case DATA:
            send_DATA_to_proc(request->addr);
            state = MOESIF_CACHE_M; //F or O or M or FM or OM would have sent this
            break;
        default:
            request->print_msg (my_table->moduleID, "ERROR");
            fatal_error ("Client: S state shouldn't see this message\n");
...
{
    switch(request->msg){
        case GETS: //are you sure?????
            set_shared_line();
            send_DATA_on_bus(request->addr, request->src_mid);
            break;
        case GETM: 
            send_DATA_on_bus(request->addr, request->src_mid);
            state = MOESIF_CACHE_FML;
            break;
        case DATA:
            fatal_error ("Should not see data for this line!  I have the line!");
            break;
...
inline void MOESIF_protocol::do_snoop_FML (Mreq *request)
{
    switch(request->msg){
        case GETS:
        case GETM: break;
        case DATA: 
            /** 
            * DATA could either be from FM (essentially F->M when others are in S)
            * or from SM (when FM exists with SM)
            */
            send_DATA_to_proc(request->addr);
            state = MOESIF_CACHE_M;
            break;
        default:
            request->print_msg (my_table->moduleID, "ERROR");
            fatal_error ("Client: FML state shouldn't see this message\n");
    }
}
...
inline void MOESIF_protocol::do_snoop_OML (Mreq *request)
{
    switch(request->msg){
        case GETS:
        case GETM: break;
