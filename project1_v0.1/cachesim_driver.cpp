...
int main(int argc, char* argv[]) {
    ...
    setup_cache(&L1, &L2, &victim, &prefetch_buffer, &markov_tag, &markov_matrix);
    ...
    while (!feof(stdin)) {
        ...
        if(ret == 2) {
            ...
            cache_access(rw, address, &stats, &L1, &L2, &victim, time, prefetch_buffer, markov_tag, markov_matrix);
        }
        //if(time==2) break;
    }
